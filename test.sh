...
COUNT=$(mysql -h 127.0.0.1 -P 3306 -u root -proot -N -e "SELECT COUNT(*) FROM information_schema.tables WHERE table_schema='CollegeDB' AND table_name IN ('Department','Faculty','Course','Student')")
...
mysql -h 127.0.0.1 -P 3306 -u root -proot CollegeDB -e "SELECT TABLE_NAME, COLUMN_NAME FROM information_schema.KEY_COLUMN_USAGE WHERE TABLE_SCHEMA='CollegeDB' AND CONSTRAINT_NAME='PRIMARY'"

echo "Checking foreign keys"

mysql -h 127.0.0.1 -P 3306 -u root -proot CollegeDB -e "SELECT TABLE_NAME, COLUMN_NAME, REFERENCED_TABLE_NAME, REFERENCED_COLUMN_NAME FROM information_schema.KEY_COLUMN_USAGE WHERE TABLE_SCHEMA='CollegeDB' AND REFERENCED_TABLE_NAME IS NOT NULL"
